...
# IMPROVED "Type" classifier: label normalization + stratified split + SMOTE (if available) + class_weight + tuning.
...
# --- optional regressors for Intensity ---
...
RANDOM_STATE = 42
CV_FOLDS = 5
N_ITER_TUNE = 60  # per regressor for intensity; and for Type tuning
...
def ensure_type_bins(label: str):
    """Normalize any free-text mode/type into low / moderate / unknown."""
    s = str(label).strip().lower()
...
# Targets (clean/binned like the app; TYPE normalized)
...
# === ONE split (STRATIFY BY TYPE to stabilize the weak task) ===
...
# Encoders for Frequency & Duration (Type we keep as strings for SMOTE)
...
# =========================
# Classification models (Freq & Dur same as before)
# =========================
...
# =========================
# TYPE — improved pipeline (SMOTE + class_weight + tuning)
# =========================
type_cv = StratifiedKFold(n_splits=CV_FOLDS, shuffle=True, random_state=RANDOM_STATE)
...
if HAS_SMOTE:
    ...
    warnings.warn("imblearn/SMOTE not available. Using class_weight='balanced' only for Type.")
...
# =========================
# Intensity — upgraded modeling (unchanged)
# =========================
...
# =========================
# Predictions for Frequency/Duration
# =========================
...
# Type uses string labels directly
t_acc, t_prec, t_f1 = print_cls_block("Type", yt_te, yt_pred)
print("\n[Type] Tuned best params:")
